...
# Try to import sphn, fall back gracefully
try:
    import sphn
    _HAS_SPHN = True
except ImportError:
...
def load_mono_audio(path: PathLike, target_sr: int) -> np.ndarray:
    ...
    if audio is None:
        try:
            ...
            raise ImportError("Neither sphn nor soundfile is available for audio loading")
    
    audio = np.asarray(audio, dtype=np.float32)
    if audio.ndim == 2:
        audio = audio.mean(axis=1)
    if sr != target_sr:
        if _HAS_SPHN and hasattr(sphn, "resample_audio"):
